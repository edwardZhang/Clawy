...
Clawy は **Tauri** ベースで構築されており、デスクトップフレームワーク自体の負担を抑えながら、より素早い起動体験を目指しています。現在の Lite パッケージは比較的小さなサイズに保たれています。

- Windows パッケージ: 約 **5.5 MB**
- macOS パッケージ: 約 **9.9 MB**
...
### OpenClaw のバージョン検出とワンクリック更新

Clawy には OpenClaw のバージョン検出機能が組み込まれており、現在のランタイムバージョン、利用可能な更新、推奨バージョンをデスクトップ上で確認し、そのまま最新バージョンへワンクリックで更新できます。

### 幅広いチャネル連携

Clawy は OpenClaw ワークフロー向けに複数のチャネル連携を統合しており、現在は次のような接続先をカバーしています。
...
Clawy は主要なモデル API 接続方式に対応しつつ、公式認証フローとサードパーティ互換 API の両方を扱えます。

- `chat-completions` と `openai-responses` ベースのサードパーティ API プロトコルをサポート
- Codex、Claude Code、MiniMax、Qwen などの OAuth 認証をサポート
- プロキシゲートウェイ、OpenAI 互換サービス、プライベートデプロイ向けにカスタムプロバイダー設定をサポート

### Token 使用量の可視化

Clawy には Token 使用量ビューアーがあり、入力・出力・総使用量をより直感的に把握できます。モデル利用コストや会話ごとの消費感覚を掴みやすくなります。

### OpenClaw の主要設定を整理し、必要に応じて開発者向け入口も提供

Clawy は OpenClaw の高頻度な設定項目をデスクトップ UI に整理し、低レベル設定ファイルを直接編集する負担を減らします。より高度な操作が必要な場合は、開発者モードから OpenClaw コントロールコンソールに入ることもできます。
