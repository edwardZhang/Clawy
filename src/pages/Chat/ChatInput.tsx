...
export function ChatInput({ onSend, onStop, disabled = false, sending = false }: ChatInputProps) {
  ...
  const pickFiles = useCallback(async () => {
    try {
      const result = await desktopApi.ipcRenderer.invoke('dialog:open', {
      ...
      const staged = await desktopApi.ipcRenderer.invoke(
        'file:stage',
        result.filePaths,
      ) as Array<{
        id: string;
  ...
  const stageBufferFiles = useCallback(async (files: globalThis.File[]) => {
    for (const file of files) {
      ...
      try {
        ...
        const staged = await desktopApi.ipcRenderer.invoke('file:stageBuffer', {
          base64,
          fileName: file.name,
          mimeType: file.type || 'application/octet-stream',
        }) as {
