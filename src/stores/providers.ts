...
export const useProviderStore = create<ProviderState>((set, get) => ({
  ...
  addProvider: async (config, apiKey) => {
    try {
      ...
      const result = await desktopApi.ipcRenderer.invoke('provider:save', fullConfig, apiKey) as { success: boolean; error?: string };
  ...
  updateProvider: async (providerId, updates, apiKey) => {
    try {
      ...
      const result = await desktopApi.ipcRenderer.invoke('provider:save', updatedConfig, apiKey) as { success: boolean; error?: string };
  ...
  deleteProvider: async (providerId) => {
    try {
      const result = await desktopApi.ipcRenderer.invoke('provider:delete', providerId) as { success: boolean; error?: string };
  ...
  setApiKey: async (providerId, apiKey) => {
    try {
      const result = await desktopApi.ipcRenderer.invoke('provider:setApiKey', providerId, apiKey) as { success: boolean; error?: string };
  ...
  deleteApiKey: async (providerId) => {
    try {
      const result = await desktopApi.ipcRenderer.invoke('provider:deleteApiKey', providerId) as { success: boolean; error?: string };
  ...
  setDefaultProvider: async (providerId) => {
    try {
      const result = await desktopApi.ipcRenderer.invoke('provider:setDefault', providerId) as { success: boolean; error?: string };
  ...
  getApiKey: async (providerId) => {
    try {
      return await desktopApi.ipcRenderer.invoke('provider:getApiKey', providerId) as string | null;
    } catch {
      return null;
    }
  },
