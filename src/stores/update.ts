...
export const useUpdateStore = create<UpdateState>((set, get) => ({
  ...
  init: async () => {
    ...
    // Get current version
    try {
      const version = await desktopApi.ipcRenderer.invoke('update:version');
    ...
    // Get current status
    try {
      const status = await desktopApi.ipcRenderer.invoke('update:status') as {
        status: UpdateStatus;
        info?: UpdateInfo;
    ...
    desktopApi.ipcRenderer.on('update:status-changed', (data) => {
    ...
    desktopApi.ipcRenderer.on('update:auto-install-countdown', (data) => {
    ...
    if (autoDownloadUpdate) {
      desktopApi.ipcRenderer.invoke('update:setAutoDownload', true).catch(() => {});
    }
  ...
  installUpdate: () => {
    desktopApi.ipcRenderer.invoke('update:install');
  },

  cancelAutoInstall: async () => {
    try {
      await desktopApi.ipcRenderer.invoke('update:cancelAutoInstall');
    } catch (error) {
  ...
  setChannel: async (channel) => {
    try {
      await desktopApi.ipcRenderer.invoke('update:setChannel', channel);
    } catch (error) {
  ...
  setAutoDownload: async (enable) => {
    try {
      await desktopApi.ipcRenderer.invoke('update:setAutoDownload', enable);
    } catch (error) {
