...
export const useGatewayStore = create<GatewayState>((set, get) => ({
  ...
  init: async () => {
    ...
    gatewayInitPromise = (async () => {
      ...
      try {
        ...
        // Listen for errors
        desktopApi.ipcRenderer.on('gateway:error', (error) => {
        ...
        desktopApi.ipcRenderer.on('gateway:notification', (notification) => {
        ...
        desktopApi.ipcRenderer.on('gateway:chat-message', (data) => {
          try {
        ...
        desktopApi.ipcRenderer.on('gateway:message', (data) => {
  ...
  start: async () => {
    try {
      ...
      const result = await desktopApi.ipcRenderer.invoke('gateway:start') as { success: boolean; error?: string };
  ...
  stop: async () => {
    try {
      await desktopApi.ipcRenderer.invoke('gateway:stop');
  ...
  restart: async () => {
    try {
      set({ status: { ...get().status, state: 'starting' }, lastError: null });
      const result = await desktopApi.ipcRenderer.invoke('gateway:restart') as { success: boolean; error?: string };
  ...
  checkHealth: async () => {
    try {
      const result = await desktopApi.ipcRenderer.invoke('gateway:health') as {
        success: boolean;
        ok: boolean;
        error?: string;
        uptime?: number
  ...
  rpc: async <T>(method: string, params?: unknown, timeoutMs?: number): Promise<T> => {
    const result = await desktopApi.ipcRenderer.invoke('gateway:rpc', method, params, timeoutMs) as {
      success: boolean;
      result?: T;
      error?: string;
    };
