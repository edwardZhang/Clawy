...
const IMAGE_CACHE_KEY = 'clawy:image-cache';
...
async function loadMissingPreviews(messages: RawMessage[]): Promise<boolean> {
  ...
  try {
    const thumbnails = await desktopApi.ipcRenderer.invoke(
      'media:getThumbnails',
      needPreview,
...
export const useChatStore = create<ChatState>((set, get) => ({
        ...
        if (sessionsToLabel.length > 0) {
          void Promise.all(
            sessionsToLabel.map(async (session) => {
              try {
                const r = await desktopApi.ipcRenderer.invoke(
                  'gateway:rpc',
                  'chat.history',
  ...
  deleteSession: async (key: string) => {
    ...
    try {
      const result = await desktopApi.ipcRenderer.invoke('session:delete', key) as {
        success: boolean;
        error?: string;
      };
      if (!result.success) {
  ...
  loadHistory: async (quiet = false) => {
    ...
    try {
      ...
      if (result.success && result.result) {
        ...
        // Skip main sessions (key ends with ":main") because they rely on the
        // Gateway-provided displayName instead.
  ...
  sendMessage: async (text: string, attachments?: Array<{ fileName: string; mimeType: string; fileSize: number; stagedPath: string; preview: string | null }>) => {
    ...
    try {
      ...
      if (hasMedia) {
        result = await desktopApi.ipcRenderer.invoke(
          'chat:sendWithMedia',
          {
