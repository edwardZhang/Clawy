 ...
 * Desktop-side helpers (readOpenClawJson / writeOpenClawJson) that
 * read from ~/.openclaw/openclaw.json. To avoid mocking the full desktop layer
...
beforeEach(async () => {
  tempDir = await mkdtemp(join(tmpdir(), 'clawy-test-'));
